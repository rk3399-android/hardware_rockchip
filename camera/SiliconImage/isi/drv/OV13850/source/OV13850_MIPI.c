...
)
{
    ...
    pOV13850Ctx->IsiCtx.NrOfAfAddressBytes     = 1U;
    ...
    TRACE( OV13850_ERROR, "dw9718s: VcmStartCurrent = %d, VcmRatedCurrent = %d, Step = %d, StartCurrent = %d, RatedCurrent = %d, StepMode = %d\n",
           pConfig->VcmStartCurrent, pConfig->VcmRatedCurrent,
           pOV13850Ctx->VcmInfo.Step,
           pOV13850Ctx->VcmInfo.StartCurrent, pOV13850Ctx->VcmInfo.RatedCurrent,
           pOV13850Ctx->VcmInfo.StepMode);
...
{
    OV13850_Context_t *pSensorCtx = (OV13850_Context_t *)handle;
    uint32_t vcm_movefull_t;
    RESULT result = RET_SUCCESS;
    uint8_t data = 0;
...
)
{
    ...
    TRACE( OV13850_ERROR, "dw9718s: nPosition = %d, data = 0x%02x 0x%02x\n", nPosition, data[0], data[1]);
...
)
{
    ...
    *pAbsStep = ( ((uint32_t)(data[0] & 0x03)) << 8U ) | ( ((uint32_t)data[1]) & 0xff);
    TRACE( OV13850_ERROR, "%s: dw9718s value = 0x%02x 0x%02x, step = %d\n", __FUNCTION__, data[0], data[1], *pAbsStep);
